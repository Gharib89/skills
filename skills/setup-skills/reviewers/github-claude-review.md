      ...
      - id: review
        ...
        with:
          ...
          prompt: |
            ...
            such a call is refused even when each command in it is allowed.
            Read a file with the Read tool rather than `cat`, and a file at the
            PR head through the `gh api .../contents?ref=` call above rather
            than `git`, which this round is not granted. Search a saved diff
            with `grep` and page it with the Read tool; `sed` and `awk` are
            refused.
