...
- **Auth failure that names no cause**: Azure DevOps answers a rejected token with a 302 to `_signin`, which `az` flattens into `ERROR: The requested resource requires user authentication`; `TF400813` with an all-`a` user GUID means the request went anonymous, not that the user lacks permission. Neither is a membership verdict. Read the real 401 first, feeding the token through stdin so it never sits in a process argument: `az account get-access-token --resource 499b84ac-1321-427f-aa17-267ca6975798 --query accessToken -o tsv | sed 's/^/Authorization: Bearer /' | curl -s -D- -H @- -H 'X-TFS-FedAuthRedirect: Suppress' 'https://dev.azure.com/<org>/_apis/connectionData?api-version=7.1-preview'` returns `www-authenticate: ... error="insufficient_claims"` with a `claims` challenge naming what the org's Conditional Access wants. A token `ipaddr` claim that differs from the challenge's `xms_rp_ipaddr` and from actual egress means a VPN split tunnel is routing ADO hostnames out of another IP; fix the machine, not the org. Membership itself is `GET https://app.vssps.visualstudio.com/_apis/accounts?memberId=<id>&api-version=7.1`, which needs no org access.
