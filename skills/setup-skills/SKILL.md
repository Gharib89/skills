...
- Each of the four sections Ship writes into that the template lacks after that migration: `## Special things to note`, `## Needs attention`, `## Verification` and `## Review`, in that order and before `## Attribution` where the template carries one. The order matches the order Ship writes them, and the placement is what keeps a `--section` write from landing a new heading below the attribution footer, which is the one thing the footer's placement rule forbids.
- The other three, `## Why the change`, `## Change outline` and `## Attribution`, when the template lacks them: Ship writes these at PR open and never rewrites them, so the opened body carries all seven either way, and proposing them here is what puts the rule in front of a human filling the template in by hand.
