...
- The other three, `## Why the change`, `## Change outline` and `## Attribution`, when the template lacks them: Ship fills them at PR open rather than rewriting them later, so a template missing one costs the reviewer the section but breaks nothing.
