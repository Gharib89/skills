...
The review gate is phase 4's self-review plus green CI (SKILL.md); the
reviewers under the profile's `## Reviewers` are a second pair of eyes on top of
it. So phase 7 is best-effort: ask each reviewer for a round, wait a bounded
time, triage whatever landed, and report what happened. A reviewer that answers
nothing costs one bounded wait, not an investigation. Preflight has already
parsed the blocks and refused the malformed ones, so what reaches this phase is
a list you can drive. Zero reviewers: skip the phase.

A reviewer re-reads the **whole PR** each round: treat each round as a fresh
read of the committed tree, not a conversation.
...
- **`on-push`**: every push earns a round; poll with no `--since`, which is the
  head rule, a round counting only on the current head.
...
- **`--brief` is how a round is read**: one `rounds[]` row per round (id,
  ...
  items) and one row per OPEN thread (id, `path`, `lead`, `resolved`,
  `replied`), the reviewer's rows only and the run's own replies dropped.
- **A body or `lead` ending `...[truncated]` has not been read.** Re-poll with
  `--brief --full <id>` for that round before triage; a thread's `lead` is read
  whole from the full shape's `threads[]`.
- **Findings live in the body as well as in threads.** A round whose findings
  sit on the review body is invisible from the thread list alone.
- **A round is a review with a body, or a bodiless verdict.** A reviewer's reply
  to one thread posts as a bodiless review row, which looks like the next round
...
- **A comment transport's window is its workflow run**, the one the block's
  `Workflow:` names. A run still going holds the window open to the ceiling
  `poll-pr --help` states, so a round being written is not read as silence.
  Run no `update-pr-title` between that request and its poll: the run is
  matched by the PR's title.
...
- **Triage, don't apply**, at the judgment tier, with phase 4's definition:
  ...
  versions, reject known non-issues with a one-line reason. Check repo-existence
  claims against `origin/HEAD`, and judge a claim apart from the evidence it
  cites. Check the reviewer's `Instructions:` file when a finding contradicts
  it, and cite it when declining. A valid finding outside the issue is an
  adjacent find, taking one of phase 2's three dispositions.
- **Batch fixes into one push per round.** A fix to a rule goes to every copy
  of that rule in the same batch: grep the phrase before you push and read each
  hunk back, because the reviewer re-reads the whole PR.
- **Reply in the thread**: `reply-thread <pr> <thread> --body-file` for every
  `replied: false` thread, `fixed in <sha>` or the decline and its reason.
...
- **Write each round to the Run file as you disposition it**, one line per
  ...
  and the exit's counts come from them.
...
reported there rather than handed back. A `Gating: yes` reviewer holding the
merge on a finding you declined still exits `reviewed`; the merge summary cites
the declined finding with its evidence as the override the human decides on.
...
`update-pr-body <pr> --section <name> --body-file <path>` per section, in this
order: `"Special things to note"` where the rounds grew the deviations log,
refolded by [pr-body.md](pr-body.md)'s rule; `"Needs attention"` where a round
filed or linked an issue or met a Ship defect; `Review` last, so `read-pr`
reads all three back at once. A `--section` write replaces the whole section
and writes the `## <name>` line itself, so each file carries that section's
content, rebuilt entire, without its heading.

## Fallbacks

A reviewer whose `Fallback-for:` names another stands in for it, on the runs
where that primary exits `not reviewed` (ADR 0002): a fallback is on-request
and conditional, which preflight enforces, since a reviewer that fires on every
...
- **GitHub Copilot as `on-request`**: requested under one login, reviewing under
  ...
  own name. The `copilot_code_review` rule's `review_on_push` fixes the
  trigger (`true` is `on-push`), and preflight refuses a `Trigger:` that
  disagrees with it. Out of quota, the host queues nothing and the request does
  not read back: `not reviewed: never-queued`.
