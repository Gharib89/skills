...
- **The trigger picks the landing rule, and the poll has to be told which.**
  Without `--since`, a round counts only on the **current head**: correct for
  `on-push`, where every push earns a fresh review. `--since <iso>` counts a
  round submitted at or after that time on **any** head, which is the rule for
  `on-request` and `auto-once`: they deliver one round per request and never
  re-post, so a push landing between the request and the review leaves the round
  keyed to the older head, and the head rule then waits out the full timeout for
  a review that will never come again. Pass `request-review`'s `requested_at`
  (`on-request`) or `open-pr`'s `created_at` (`auto-once`) straight through.
  `--since` without `--await-review` is a usage error.
...
for it to land with `poll-pr --await-review <login> --since <created_at>`,
the `created_at` that `open-pr` printed, because the one round it fires can land
after a push has moved the head. If a round arrives before you poll, that is the round. Triage it once, push the fixes, reply on each
...
a review **landed on the current head**: poll with no `--since`, so a round
keyed to the pushed-over head does not count. Silence on the head is never quiet.
...
request against the corrected tree. Loop: request, poll with `--since` the
`requested_at` that `request-review` just printed, triage, batch-fix, push,
reply on each thread, request again. **Converged** when the latest round
