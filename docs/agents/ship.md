...
`prose-budget` holds the shape of ship's own documents, the one gate here whose subject is prose rather than a script: `skills/*/SKILL.md` at most 400 lines, and every `skills/*/reference/*.md` over 100 lines opening with a `## Contents` heading inside its first 15 lines. A run reads `SKILL.md` whole at load and re-reads it after a compaction, so a file that outgrows the budget spends that read on reference material and reaches phase 0 late; the `## Contents` list is what makes a long reference file answerable without reading it through. `scripts/prose-budget-check.sh` is the whole rule, a script rather than a function in the gate so `tests/prose-budget.test.sh` can drive it against fixture trees, the way `derived-copies` calls `scripts/profile-schema-check.sh`.
