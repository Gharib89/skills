...
- `tests/run.sh` green, per the `tests` gate. It runs every `tests/*.test.sh`: the pure transformations the mechanics are built around, sourced and asserted on as strings, and the gate scripts, run against a fixture and asserted on their exit code. Neither reaches a host. A behavioural claim about either earns a case there.
