...
# `$LIST` is the id list; `$RULE_<id>` is that ruleset's review_on_push lines.
api() { # <path> [--jq <filter>]
  case $1 in
    */rulesets)   printf '%s' "$LIST" | tr ' ' '\n' | sed '/^$/d' ;;
    */rulesets/*) eval "printf '%s' \"\${RULE_${1##*/}-}\"" ;;
    *) return 1 ;;
  esac
}

LIST="10" RULE_10="true"
check "one active ruleset with review_on_push: true" \
  'true' "$(host_copilot_review_on_push)"

LIST="10" RULE_10="false"
