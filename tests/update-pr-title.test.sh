...
printf '{"title":"new-title"}\n'  > "$SHIP_FAKE/host_pr_get.2.json"
out=$(run new-title); rc=$?
check_rc "a title that differs exits 0" 0 "$rc"
check "the write is reported once read back as changed" 'new-title true' \
  "$(jq -r '[.title, .changed] | @tsv' <<<"$out" | tr '\t' ' ')"
check "the sequence is read, write, read back" \
  'host_pr_get 7
host_pr_set_title 7 new-title
host_pr_get 7' "$(cat "$SHIP_FAKE/calls")"

reset
printf '{"title":"same-title"}\n' > "$SHIP_FAKE/host_pr_get.1.json"
out=$(run same-title); rc=$?
check_rc "a title already equal exits 0" 0 "$rc"
check "and is reported unchanged" 'same-title false' \
  "$(jq -r '[.title, .changed] | @tsv' <<<"$out" | tr '\t' ' ')"
check "no write reaches the host" 'host_pr_get 7' "$(cat "$SHIP_FAKE/calls")"
