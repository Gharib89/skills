...
# the landing and refusal rules read the same grade on every host (#268). No
# adapter sends `substantive` (#270); the rows below carry what each adapter
# sends, and poll-pr's grade is what decides.
#
...
vote='{"id": null, "login": "voter@example.com", "state": "approved", "submitted_at": null, "body": ""}'
reset "$vote"
...
# The Azure DevOps notice-only thread: a comment row whose body is a notice.
thread=$(jq -cn --arg b "$notice_text" \
  '{id: "41", login: "voter@example.com", state: "comment", submitted_at: "2026-09-24T09:05:00Z", body: $b}')
reset "$thread"
...
check "it refuses the round instead" 'null since' \
  "$(rules <<<"$out")"
check "because poll-pr graded it not substantive" false "$(graded <<<"$out")"
