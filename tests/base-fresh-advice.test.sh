...
source tests/lib.sh
fresh="$PWD/skills/ship/scripts/base-fresh.sh"
...
check "a behind branch on origin is told to merge the base in" \
  "branch has not seen these commits on origin/main; it is on origin, so merge origin/main in rather than rebase, and re-run:" \
  "$(head -1 "$tmp/err")"
