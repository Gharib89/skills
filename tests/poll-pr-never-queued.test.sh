#!/usr/bin/env bash
# poll-pr --free-round and a round the host never queued: once a settle has
# passed since `--since`, a since-rule poll under the host's request transport
# asks the host whether the awaited reviewer has a request on record since then,
# and a `false` closes the window at once with `never_queued: true` and
# done=false. In every run observed since #266, a quota-out Copilot left no
# request event and no notice, and the free-round poll waited its whole 600 s
# window (#273) before the loop's first request read back never-queued within
# seconds (#284). A poll after a request passes no flag and is unchanged.
#
...
reset; queued false
out=$(poll --free-round --reviewer copilot --since "$old" --timeout 10 --interval 1); rc=$?
...
reset; queued true
out=$(poll --free-round --reviewer copilot --since "$old" --timeout 2 --interval 1); rc=$?
...
out=$(poll --free-round --reviewer copilot --since "$old" --interval 30); rc=$?
...
reset; queued false
out=$(poll --free-round --reviewer copilot --since "$new" --timeout 0 --interval 1)
...
out=$(poll --free-round --reviewer copilot --since "$old" --timeout 0 --interval 1)
...
reset; queued false
out=$(poll --free-round --reviewer claude --since "$old" --timeout 0 --interval 1)
check "under the comment transport the read is never made" 0 "$(calls pr_review_queued)"
check "and never_queued stays null" null "$(jq -r .never_queued <<<"$out")"

# A poll after a request passes no --free-round: it keeps today's window and
# makes no queued read, whatever the host would have answered.
reset; queued false
out=$(poll --reviewer copilot --since "$old" --timeout 0 --interval 1)
check "without --free-round the read is never made" 0 "$(calls pr_review_queued)"
check "and never_queued stays null" null "$(jq -r .never_queued <<<"$out")"

# --free-round keys the read to --since, so it needs one.
reset
out=$(poll --free-round --reviewer pusher); rc=$?
check_rc "--free-round with no --since is a usage error" 2 "$rc"
check "reaching no host" false "$([ -s "$SHIP_FAKE/calls" ] && echo true || echo false)"
...
out=$(poll --free-round --reviewer copilot --since "$old" --timeout 60 --interval 30); rc=$?
