...
- **`file-issue` it** with the profile's triage marker and leave it. The
  ...
  same finding is linked in the deviations log rather than refiled, with any
  evidence this run adds posted on that issue by `comment-issue`, and a
  different one is refiled with `--distinct-from`.
