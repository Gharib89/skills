...
section: `class docs` and `small lane` skip the phase; a full-lane change that
no `Applies when:` matches is the third reason, a profile with zero
verifications included. On an `unexercised` row, `<what ran>` names the
