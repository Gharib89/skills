...
- **An allowlist that covers every step of the prompt.** A tool the round needs and cannot call is a denied call and a spent turn, so the list is derived from what the prompt's own steps ask for, plus the read-only verbs a round reaches for while sizing a large diff, and the denial step below names any call it still misses. `Read`, `Grep` and `Glob` serve step 1's brief and standards; `Bash(gh pr view:*)` and `Bash(gh issue view:*)` step 2's spec; `Bash(gh pr diff:*)` step 3's diff; `Bash(gh api:*)` the POST and, in the fallback shape, the PR's copy of a changed file; and `Bash(head:*)`, `Bash(tail:*)` and `Bash(wc:*)` because a pipe is refused unless **both** of its commands are granted, so `gh pr diff | wc -l` needs `wc` named too. Those three size a diff and leave the reviewing to step 3: step 3 says review what the one `gh pr diff` returned, and a `head` that truncates it leaves the tail unreviewed with nothing saying so. `Bash(grep:*)` is there for the diff Claude Code saves rather than returns: past about 30 KB, `gh pr diff` comes back as the path of a `tool-results/*.txt` file, and a round slices that file to find its `diff --git` headers (the Read tool's offset and limit page through it too). `sed` and `awk` stay denied on purpose, though a round reaches for them on that same file: measured 2026-09-24 on Claude Code 2.1.282, `Bash(sed:*)` admitted `sed -n '1e touch PWNED_sed' probe.txt` and ran the embedded command, and while the harness refused awk's `system()`, awk has other ways to execute. The round reads untrusted diff text, on a public repo in reach of any commenter's `@claude` under the fallback shape, with the Claude OAuth token and a `pull-requests: write` token in the runner, so a verb that can execute is a shell an injected prompt gets. Deliberately no `git` entry, though [`ado-claude-review.md`](ado-claude-review.md) grants two: that prompt reaches for `git diff` because Azure DevOps has no `gh`, while both shapes here check out at `fetch-depth: 1` and read the diff through `gh pr diff`, so `git log` would have no history to read and neither prompt asks for either. Nothing here writes to the checkout, the one write it can make anywhere is the review POST the round exists for, and nothing reaches the network beyond `gh`. Narrow it and the round spends its turns on denied calls and posts nothing.
        ...
        with:
          ...
          prompt: |
            ...
            3. Run `gh pr diff ${{ github.event.pull_request.number }}` once and review
               ...
               and the round dies before it posts. A large diff comes back as the
               path of the file it was saved to: page through that file with the
               Read tool's offset and limit, or search it with `grep`, and review
               all of it. To see a changed file beyond its hunks, Read it from the
               checkout, which is the PR's merge ref.
        ...
        with:
          ...
          prompt: |
            ...
            3. Run `gh pr diff ${{ github.event.issue.number }}` once and review
               ...
               path of the file it was saved to: page through that file with the
               Read tool's offset and limit, or search it with `grep`, and review
               all of it. To see a changed file beyond its hunks, read the PR's
               copy with
               `gh api 'repos/${{ github.repository }}/contents/<path>?ref=<head sha>' -H 'Accept: application/vnd.github.raw+json'`,
               the head sha from `gh pr view ${{ github.event.issue.number }} --json headRefOid`:
               the checkout is the default branch, so a Read there is its copy,
               not the PR's.
