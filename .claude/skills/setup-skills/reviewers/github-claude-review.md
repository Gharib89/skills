    ...
    steps:
      ...
      - id: review
        ...
        with:
          ...
          prompt: |
            ...
            value (an apostrophe inside one is written '\'' ). Four traps:

            - Use `-F` for every field, never `-f`: gh sends every `-f` field
              ahead of every `-F` one, so mixing them detaches a `line` from its
              comment.
            - A value starting with `@` is read as a file path. Never start one
              with `@`.
            - `{owner}`, `{repo}` and `{branch}` in a value are filled in from the
              checkout. Name a placeholder in words rather than quoting it.
            - A value of exactly `true`, `false`, `null` or an integer becomes
              that JSON type, which is what makes `line` an integer.

            Do not send the review as JSON on stdin: Claude Code's Bash check
            refuses a heredoc of JSON before `--allowedTools` is read.
            `--allowedTools` below grants `Bash(gh api:*)`, which matches a
            command that starts with `gh api` and nothing else, so piping from
            `echo` makes `echo` the command and is refused too. A refused call
            is a round that stops before the PR.
      ...
      - if: always() && steps.review.outputs.execution_file != ''
        ...
        run: |
          ...
          denials="$(jq -rs 'flatten | map(select(.type == "result")) | last
                             | .permission_denials // [] | .[]
                             | "\(.tool_name) \(.tool_input | tojson | .[0:300])"
                             | gsub("#"; "\\u0023")' "$EXECUTION_FILE" 2>/dev/null || true)"
          [ -n "$denials" ] || exit 0
          n=$(printf '%s\n' "$denials" | wc -l)
...
4. Read the first round's denials: its run log's `denied:` lines, or the absence of the warning. For each one, either widen `--allowedTools` to admit the call or record beside `--allowedTools` why it stays denied, so every denial has an owner from the first round on.
...
4. Read that round's denials: its run log's `denied:` lines, or the absence of the warning. For each one, either widen `--allowedTools` to admit the call or record beside `--allowedTools` why it stays denied, so every denial has an owner from the first round on.
