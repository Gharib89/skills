...
- **PR body: the Summary opens with a Shape.** A `diff` fence over a call tree, file tree, control flow, pseudocode or component tree, first thing under `## Summary`, or first in the body where no template gives that heading. Text forms only, so the shape reads in a plain diff: mermaid and HTML are out. One shape, about 15 lines or fewer. Every node is a real symbol, each tree's root node carries its file path, and no line carries a line number. A change that moves no logic and no layout opens with `Shape: none, mechanical (<kind>).` instead; silent absence is a finding.
