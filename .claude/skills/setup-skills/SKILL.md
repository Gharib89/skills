...
- Each of the four sections Ship writes into that the template lacks: `## Special things to note`, `## Needs attention`, `## Verification` and `## Review`, in that order and before `## Attribution` where the template carries one. The order matches the order Ship writes them, and the placement is what keeps a `--section` write from landing a new heading below the attribution footer, which is the one thing the footer's placement rule forbids.
