...
- A **closing-reference move**, when a closing reference sits below the first `## ` heading: propose moving that line, unchanged, above the first heading. A closing reference is any inflection of `close`, `fix` or `resolve` followed by `#<issue>`, case-insensitive, the set `ship_body_closes` reads as a claim on the issue. A section rewrite drops whichever one sits inside a section, and nothing else migrates a template written before the reference moved out of `## Summary`.
